...
### Changed — gd
- `check_level.py` LV3: если цель достижима только через `optional`-связи, скрипт называет эти связи и просит пометить их `critical`. Раньше он писал просто «goal недостижим». `level-method.md`: ветка до ключа гейта на критическом пути — тоже `critical`.

### Fixed — gd-build
- `check_audio_files.py` AF5: источник `made` больше не считается внешним. Свой файл с лицензией own проходит без URL.
- `check_audio_files.py`: если `--ffmpeg` указывает на несуществующий файл, шапка пишет «нет ffmpeg — sample peak». Раньше она писала «ffmpeg», хотя пик считался как sample peak.
- `check_release.py`: если проект не внутри `--repo`, скрипт выходит с кодом 2 и сообщением. Раньше падал трейсбеком `ValueError`.
- `compare_perf.py`: описание PF4 в докстринге приведено к коду.

### Tooling
- `tools/test_scripts.py`: +1 тест (LV3 при ключе за optional-связью), расширены тесты `check_audio_files.py` и `check_release.py`. Всего 64.
