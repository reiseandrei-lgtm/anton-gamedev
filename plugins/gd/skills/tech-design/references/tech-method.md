...
|---|---|---|---|---|---|---|
| B1 | Кадр | 16.6 ms (60 fps) | mid Android | … | Profiler, 60 с игры | стабильно > 14 ms → резать |
```

ID `B1…` стабильны: на них ссылаются perf-кейсы тест-плана (`budgets#B1`) и отчёты `gd-build: perf-check` в `qa/perf/`. Имя метрики в отчёте = имя в этой таблице.
