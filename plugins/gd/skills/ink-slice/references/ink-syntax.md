> Copied from spaceninja/narrative-ink-skills `ink-syntax/SKILL.md` (MIT © 2026 Scott Vandehey), itself summarizing inkle's "Writing with ink" (MIT © inkle Ltd). Modified: this header and one internal link path. See ATTRIBUTION.md.
...
For complete documentation on advanced list operations, multi-list state machines, threads, and the full weave syntax, see [reference.md](ink-syntax-extended.md).
